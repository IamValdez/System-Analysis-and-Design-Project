				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								...
								<select name="department_ids[]" id="" multiple=""  class="custom-select browser-default select2">
									<option value=""></option>
									<?php 
									$qry = $conn->query("SELECT * FROM department_specialty order by name asc");
							...
							<div class="form-group">
								...
								<textarea name="college_department" id="" cols="30" rows="2" class="form-control" required=""></textarea>
							</div>
							<div class="form-group">
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$cats = $conn->query("SELECT * FROM facultys_list order by id asc");
								...
								<tr>
									...
									<td class="">
										 ...
										 <p><small>Department: <b><?php echo $row['college_department'] ?></b></small></p>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit-faculty" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-name_pref="<?php echo $row['name_pref'] ?>" data-college_department="<?php echo $row['college_department'] ?>" data-contact="<?php echo $row['contact'] ?>"  data-img_path="<?php echo $row['img_path'] ?>" data-department_ids="<?php echo $row['department_ids'] ?>" data-email="<?php echo $row['email'] ?>">Edit</button>
...
<script>
		...
		if($(this).attr('data-department_ids')!= ''){
			...
			ids.map(function(e){
				nids.push(e)
					...
					$('[name="department_ids[]"]').val(nids)
		}else{
			$('[name="department_ids[]"]').val('')

		}
			$('[name="department_ids[]"]').trigger('change')
		// $('[name="department_ids[]"]').select2({
		// 	placeholder:"Please Select Here",
		// 	width:'100%'
		// })
		end_load()
	...
	$('.view_schedule').click(function(){
		uni_modal($(this).attr('data-name')+" - Schedule","view_faculty_schedule.php?id="+$(this).attr('data-id'))
	})
